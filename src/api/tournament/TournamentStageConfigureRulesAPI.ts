...
export class TournamentStageConfigureRulesAPI extends APIMessage<TournamentSummaryView> {
  ...
  constructor(tournamentId: string, stageId: string, request: ConfigureStageRulesRequest) {
    ...
    this.request = {
      ...
      format: request.format ? [request.format] : emptyBackendOption<string>(),
      roundCount: request.roundCount === undefined ? emptyBackendOption<number>() : [request.roundCount],
      advancementRuleType: request.advancementRuleType ? [request.advancementRuleType] : emptyBackendOption<string>(),
      ...
      pairingMethod: request.pairingMethod ? [request.pairingMethod] : emptyBackendOption<string>(),
      ...
      seedingPolicy: request.seedingPolicy ? [request.seedingPolicy] : emptyBackendOption<string>(),
