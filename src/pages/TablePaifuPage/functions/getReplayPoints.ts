...
export function getCurrentRiichiStickCount({
  ...
  return (
    ...
    getAcceptedRiichiActions(round, getReplaySequenceLimit(round, replayStep))
      .length
  );
}
...
export function getCurrentPlayerPoints({
  ...
  return (
    ...
    (selectedRound.result.scoreChanges.find(
...
export function getPlayerPointsBeforeSettlement({
  ...
  const initialPoints =
    paifu.metadata.seats?.find((item) => item.playerId === playerId)
      ?.initialPoints ?? 0;
  ...
  const completedPoints = completedRounds.reduce((total, round) => {
    ...
    return (
      total + scoreChange + getRoundRiichiPaymentForPlayer(round, playerId)
    );
  }, initialPoints);
