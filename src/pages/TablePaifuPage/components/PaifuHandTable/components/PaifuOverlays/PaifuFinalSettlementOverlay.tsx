...
export function PaifuFinalSettlementOverlay({
  onConfirm,
  paifu,
}: PaifuFinalSettlementOverlayProps) {
  const playerNames = paifu.metadata.playerNames ?? {};
  const standings = [...paifu.finalStandings].sort(
    (left, right) =>
      left.placement - right.placement || right.finalPoints - left.finalPoints,
  );
          ...
          {standings.map((standing) => (
            ...
            >
              ...
              <span className="justify-self-end text-right tabular-nums text-[#f2f7fb]">
                {formatPoints(standing.finalPoints)}
              </span>
            </div>
          ))}
        </div>
